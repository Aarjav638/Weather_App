...
export const addCityDetails = async (db: SQLiteDatabase, cityDetails: CityDetails) => {
    const insertQuery = `
  ...
  export const getCityDetail = async (db: SQLiteDatabase): Promise<CityDetails[]> => {
    try {
...
//     const updateQuery = `
//       UPDATE cityDetails
//       SET temperature = ?, airQuality = ?, cityName = ?, date = ?, weatherDetails = ?
//       WHERE id = ?
//     `;
//     const values = [
//         cityDetails.temperature,
