...
const ManageCities = () => {
    ...
    const getLocations = async (searchQuery: string) => {
        try {
            const response = await axios.get(`https://api.geoapify.com/v1/geocode/autocomplete?text=${searchQuery}&format=json&apiKey=07a421da47de4677978182f0c6246538`);
    ...
    return (
        <View style={{ flex: 1 }}>
            ...
            <Modal isVisible={isModalVisible} deviceWidth={Dimensions.get('window').width} style={{ margin: 0, marginTop: '2%' }} deviceHeight={Dimensions.get('window').height} backdropOpacity={0.9}>
                <View style={styles.container}>
                    <View style={styles.maincontainer}>
                        <Text onPress={()=>setModalVisible(false)} style={styles.cancel}>Cancel</Text>
                    ...
                    <View style={styles.flatview}>
                        <FlatList
                            ...
                            renderItem={({ item }) =>
                                item.city ? (
                                    <View>
                                        <Text onPress={() => { setCity(item.city); setModalVisible(false); }} style={{ color: 'black', fontSize: 18, fontFamily: 'Poppins-Medium', marginBottom: 10 }}>
                                            {item.city}
                                        </Text>
                                    </View>
                                ) : null
